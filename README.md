# Set Test Name in Pytest with Selenium on TestMu AI (Formerly LambdaTest)
...
With TestMu AI (Formerly LambdaTest), you can programmatically set a custom test name for each automation test run so results are easy to identify in the dashboard. This sample shows how to configure test name in Pytest on the TestMu AI cloud.
...
To set the test name as the method name in Pytest, add the following line to the driver function in the `conftest.py` file (full code can be referred in `conftest.py`):
